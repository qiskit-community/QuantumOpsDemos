...
#  to see if the implementation could be fairly efficient while not requiring too
...
# * `OpSum` a parametric type representing a sum of `OpTerm`s. `OpSum` is implemented as an `Vector`
...
#    than Qiskit. [Note: This was before the series of improvements to `SparsePauliOp`.]
...
# *  A package `qisit_alt` provides a demonstration of wrappers allowing Qiskit classes as input and output.
#
...
# We won't consider `PauliI` in this document.
#
...
#
# `OpTerm` is a parametric type with three parameters.
# The first is an operator type `<:AbstractOp`.
# The second specifies the type of storage for the string of operators.
# The third is the type of the coefficient.
...
# By default, the second parameter is a dense `Vector`. For the concrete operator types that we
